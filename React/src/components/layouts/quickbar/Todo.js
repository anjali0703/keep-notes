...
const addtasktip = <Tooltip>Add a Task to this block</Tooltip>;
...
class Todo extends Component {
  ...
  handleSubmit(e) {
    ...
    if (this.state.tasktitle !== "" && this.state.tasktitle !== undefined) {
      this.state.taskbox.unshift({
        maintitle: this.state.tasktitle,
        tasklist: [],
      });
  ...
  handleSubmitTask(e) {
    ...
    if (this.state.task !== "" && this.state.task !== undefined) {
      this.state.taskbox[this.state.lastActiveBox].tasklist.push({
        title: this.state.task,
        state: false,
      });
      this.setState({
        tasklist: this.state.taskbox[this.state.lastActiveBox].tasklist,
      });
    }
  ...
  render() {
    // task list
    this.state.taskbox.tasklist ||
      [].map((task, i) => (
        <li
          key={i}
          className={
            task.state
              ? "ms-list-item ms-to-do-task ms-deletable task-complete"
              : "ms-list-item ms-to-do-task ms-deletable"
          }
        >
          <label className="ms-checkbox-wrap ms-todo-complete">
            <input
              type="checkbox"
              onClick={(e) => this.toggleTaskState(task)}
            />
          ...
          <button
            type="submit"
            className="close"
            onClick={(event) => this.removeTask(i, event)}
          >
    ...
    const taskbox = (this.state.taskbox || []).map((taskBox, index) => (
      <li key={index} className="ms-card ms-qa-card ms-deletable">
        <div className="ms-card-header clearfix">
          ...
          <OverlayTrigger placement="left" overlay={addtasktip}>
            <button className="ms-add-task-to-block ms-btn-icon float-right">
              {" "}
              <i
                className="material-icons text-disabled"
                onClick={(e) => this.handleShow(index)}
              >
                add
              </i>{" "}
            </button>
        ...
        <div className="ms-card-footer clearfix">
          <Link to="#" className="text-disabled mr-2">
            {" "}
            <i className="flaticon-archive"> </i> Archive{" "}
          </Link>
          <Link
            to="#"
            className="text-disabled  ms-delete-trigger float-right"
            onClick={(event) => this.removeTaskbox(index, event)}
          >
            {" "}
    ...
    return (
      <div className="ms-quickbar-container ms-todo-list-container ms-scrollable">
        <form className="ms-add-task-block" onSubmit={this.handleSubmit}>
          <div className="form-group mx-3 mt-0  fs-14 clearfix">
            <input
              type="text"
              className="form-control fs-14 float-left"
              name="tasktitle"
            ...
            <button
              type="submit"
              className="ms-btn-icon bg-primary float-right"
            >
              <i className="material-icons text-disabled">add</i>
            </button>
          </div>
        </form>
        <ul className="ms-todo-list">
          {this.state.tasktitle && (
            <li className="ms-card ms-qa-card ms-deletable">
              ...
              <div className="ms-card-body">
                <ul className="ms-list ms-task-block"></ul>
              </div>
              <div className="ms-card-footer clearfix">
                <Link to="#" className="text-disabled mr-2">
                  {" "}
